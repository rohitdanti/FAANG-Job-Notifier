...
async def _go_to_page(page, target_page: int) -> None:
    ...
    while current_page < target_page:
        ...
        if not await next_button.count():
            raise RuntimeError(f"Unable to find CVS Workday pagination control for page {target_page}")
...
COMPANY = CompanyDefinition(
    slug="cvs_wd",
    display_name="CVS Health (Workday)",
    default_search_url=CVS_WD_SEARCH_URL,
    default_max_pages=2,
